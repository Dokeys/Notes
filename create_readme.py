...
def read_and_append_header() -> None:
    ...
    try:
        ...
        print_error(f"Header Datei: {HEADER_FILE} nicht gefunden!")
        exit
...
def get_directory_folders() -> list[str]:
    folders = [
        f
        for f in os.listdir(NOTES_DIRECTORY)
        if os.path.isdir(os.path.join(NOTES_DIRECTORY, f))
    ]
...
def get_filenames_from_folder(folder: str) -> list[str]:
    ...
    filenames = [
        filename for filename in filenames if filename.endswith(USED_FILE_TYPES)
    ]
    return filenames
...
def append_filename_links(filenames: list[str], folder: str) -> None:
    with open(OUTPUT_FILE, "a", encoding="utf-8") as file:
        for filename in filenames:
            ...
            link_string = (
                f"- [{filename_without_ending}]({REPOSITORY_LINK}{folder}/{filename})\n"
            )
